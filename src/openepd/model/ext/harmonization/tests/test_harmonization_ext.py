...
class HarmonizationExtTestCase(unittest.TestCase):
    def test_parse_coproducts_extension_from_epd(self) -> None:
        ...
        self.assertEqual(ext.coproducts.impacts["TRACI 2.1"].root["gwp"].A1A2A3.qty, 1.23)
        self.assertEqual(ext.coproducts.impacts["TRACI 2.1"].root["gwp"].A1A2A3.unit, "kgCO2e")
    ...
    def test_coproducts_rejects_invalid_enum_values(self) -> None:
        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"coproducts": {"allocation_basis": "not-valid"}})

        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"coproducts": {"base_allocation_basis": "not-valid"}})

    def test_multiply_after_item_length_is_limited(self) -> None:
        with self.assertRaises(pyd.ValidationError) as context:
            HarmonizationEpdExtension.model_validate(
                {
                    "eac": {
                        "harmonization_factors": True,
    ...
    def test_multiply_after_item_length_is_reflected_in_schema(self) -> None:
        schema = EacHarmonizationFactor.model_json_schema()
    ...
    def test_parse_eac_extension_from_epd(self) -> None:
        ...
        self.assertEqual(ext.eac.impacts["TRACI 2.1"].root["gwp"].A1A2A3.qty, 3.2)
        self.assertEqual(ext.eac.impacts["TRACI 2.1"].root["gwp"].A1A2A3.unit, "kgCO2e")
        self.assertEqual(ext.eac.impacts["TRACI 2.1"].root["gwp"].A4.qty, -0.1)
        self.assertEqual(ext.eac.resource_uses.root["rpre"].A1A2A3.qty, -2.0)
        self.assertEqual(ext.eac.resource_uses.root["rpre"].A1A2A3.unit, "MJ")
        self.assertEqual(ext.eac.output_flows.root["hwd"].A1A2A3.qty, -0.05)
        self.assertEqual(ext.eac.output_flows.root["hwd"].A1A2A3.unit, "kg")
    ...
    def test_eac_rejects_invalid_enum_values(self) -> None:
        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"eac": {"EAC_standard": "Not a valid standard"}})

        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"eac": {"EAC_coverage": "Not a valid coverage"}})
    ...
    def test_parse_mbvfa_extension_from_epd(self) -> None:
        ...
        self.assertEqual(ext.mbvfa.impacts["TRACI 2.1"].root["gwp"].A1A2A3.qty, -1.23)
        self.assertEqual(ext.mbvfa.impacts["TRACI 2.1"].root["gwp"].A1A2A3.unit, "kgCO2e")
    ...
    def test_mbvfa_rejects_invalid_enum_values(self) -> None:
        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"mbvfa": {"tracking_approach": "not-valid"}})

        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"mbvfa": {"attribute": "not-valid"}})
    ...
    def test_parse_offsets_extension_from_epd(self) -> None:
        ...
        self.assertEqual(ext.offsets.impacts["TRACI 2.1"].root["gwp"].A1A2A3.qty, -12.34)
        self.assertEqual(ext.offsets.impacts["TRACI 2.1"].root["gwp"].A1A2A3.unit, "kgCO2e")
    ...
    def test_offsets_reject_invalid_enum_values(self) -> None:
        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"offsets": {"offset_standards": ["Not a valid standard"]}})

        with self.assertRaises(pyd.ValidationError):
            HarmonizationEpdExtension.model_validate({"offsets": {"offset_coverage": "Not a valid coverage"}})

    def test_custom_factor_extra_is_allowed_and_typed(self) -> None:
        ext = HarmonizationEpdExtension.model_validate(
            {
        ...
        extra = none_raises(ext.model_extra)
        self.assertIn("biochar", extra)
        self.assertIsInstance(extra["biochar"], CustomHarmonizationFactor)
        custom_factor = extra["biochar"]
        ...
        self.assertEqual(custom_factor.model_extra["internal_method"], "v1")
    ...
    def test_custom_factor_extra_accepts_preparsed_model(self) -> None:
        ...
        extra = none_raises(ext.model_extra)
        self.assertIn("biochar", extra)
        self.assertIs(extra["biochar"], custom_factor)
        self.assertEqual(extra["biochar"].name, "Biochar sequestration accounting")
        self.assertEqual(extra["biochar"].multiply_after, ["coproducts"])
        self.assertEqual(extra["biochar"].model_extra["internal_method"], "v1")
    ...
    def test_custom_factor_extra_not_typed_for_non_object_payload(self) -> None:
        ...
        errors = context.exception.errors()
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0]["loc"], ("biochar",))
        self.assertEqual(
            errors[0]["msg"], "Input should be a valid dictionary or instance of CustomHarmonizationFactor"
        )
        self.assertEqual(errors[0]["type"], "model_type")
    ...
    def test_custom_factor_missing_required_name_raises_validation_error(self) -> None:
        ...
        errors = context.exception.errors()
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0]["loc"], ("biochar", "name"))
