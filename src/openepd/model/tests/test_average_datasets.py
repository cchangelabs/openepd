...
class AverageDatasetTestCase(unittest.TestCase):
    def test_generic_estimate_doctype(self):
        ...
        self.assertEqual(
            GenericEstimatePreview.parse_obj(
                {**GE_REQUIRED_FIELDS, "id": "0197ad82-92cf-7978-a6c8-d4964c0a3624"}
            ).doctype,
            "openGenericEstimate",
        )
...
class GECompositionTestCase(unittest.TestCase):
    def test_generic_estimate_composition_constituent_bounds(self) -> None:
        ...
        ge = GenericEstimatePreview.parse_obj({**GE_REQUIRED_FIELDS, "composition": [valid_constituent.dict()]})
        self.assertEqual(len(ge.composition), 1)

        # Invalid: kg_C_biogenic > kg_C
        invalid_constituent_1 = valid_constituent.dict()
        invalid_constituent_1["kg_C_biogenic"] = 15.0
        invalid_constituent_1["kg_C"] = 10.0
        with self.assertRaises(pyd.ValidationError) as e:
            GenericEstimatePreview.parse_obj({**GE_REQUIRED_FIELDS, "composition": [invalid_constituent_1]})
        self.assertIn("kg_C_biogenic must be less than or equal to kg_C", str(e.exception))

        # Invalid: kg_post_consumer_recycled > kg_mass
        invalid_constituent_2 = valid_constituent.dict()
        invalid_constituent_2["kg_post_consumer_recycled"] = 25.0
        invalid_constituent_2["kg_mass"] = 20.0
        with self.assertRaises(pyd.ValidationError) as e:
            GenericEstimatePreview.parse_obj({**GE_REQUIRED_FIELDS, "composition": [invalid_constituent_2]})
        ...
        invalid_constituent_3 = valid_constituent.dict()
        invalid_constituent_3["kg_pre_consumer_recycled"] = 25.0
        invalid_constituent_3["kg_mass"] = 20.0
        with self.assertRaises(pyd.ValidationError) as e:
            GenericEstimatePreview.parse_obj({**GE_REQUIRED_FIELDS, "composition": [invalid_constituent_3]})
    ...
    def test_generic_estimate_lci_databases_and_software(self) -> None:
        ...
        ge = GenericEstimatePreview.parse_obj(
            {
                **GE_REQUIRED_FIELDS,
        ...
        ge_with_ref = GenericEstimatePreview.parse_obj(
            {
                **GE_REQUIRED_FIELDS,
