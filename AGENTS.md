...
Python library (v6.x) for the [openEPD](https://www.buildingtransparency.org/programs/openepd/) data format —
Environmental Product Declarations for building materials. The project targets Pydantic v2 but includes an
internal compatibility/patch layer to retain some pydantic.v1-style behaviors where necessary (see
`src/openepd/patch_pydantic.py` and `src/openepd/compat/pydantic.py`). Provides Pydantic models, an API client,
a bundle format, and a hierarchical category system.
